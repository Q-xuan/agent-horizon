...
需要在仓库 Settings → Secrets 里加 **一个** 模型密钥，和 `data/config.github.json` 里的 `api_key_env` 对上。Actions 当前用 OpenAI 兼容网关 + `gemini-3.7-flash`：

- `OPENAI_API_KEY`

接口地址在 `ai.base_url`，默认 `https://ai.aruyx.com/cpa/v1`。
