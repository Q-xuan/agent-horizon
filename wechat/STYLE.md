...
frontmatter 的 `source:` 填 `https://q-xuan.github.io/agent-horizon/`，作为文末「阅读原文」直达链接。个人订阅号正文外链受微信限制无法直接在正文点击跳转，正文里的 `原文：[标题](https://...)` 供出处溯源展示，文末「阅读原文」是最稳的外链，直达完整的 Horizon 每日报告。没有 `source:` 时，用正文里第一条 `原文：[…](https://…)`；再没有，才用正文第一条 `https://`。
...
4. `> **手记**：...`（1~2 句硬核技术洞察或工程影响推演）
5. `原文：[标题](https://...)`（极简外链脚注）
