...
class ProcessDocsTests(unittest.TestCase):
    def test_template_and_guides_spell_yuanywen(self) -> None:
        ...
        push_src = (root / "scripts" / "push_draft.py").read_text(encoding="utf-8")
        write_src = (root / "scripts" / "write_from_digest.py").read_text(encoding="utf-8")
        self.assertIn('or "yuseus"', push_src)
